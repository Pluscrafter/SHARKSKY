...
I(t)=\frac{dQ_{c}}{dt}=C*\frac{dv_{out}}{dt}
\end{equation}
...
Jetzt wird $v_{\text{in}}(t)$ mit $x_{i}$ und $v_{out}$ mit $y_{i}$ dargestellt und die Differenzialgleichung diskretisiert zu der Differenzengleichung \ref{eq4}.
...
Die Gleichung wird nach $y_{i}$ umgeformt und man kann $\sfrac{\Delta t}{\Delta t+RC}$  mit $\alpha$ und $\sfrac{RC}{\Delta t + RC}$ mit $\alpha-1$ substituieren wie in \ref{eq5} gezeigt.
...
Jetzt kann auch das $RC$ in der Definition von $\alpha$ ersetzt werden, damit die Gleichung nicht mehr von $RC$ abhängig ist.
...
\begin{equation}
w=\omega*dt \quad mit \quad w=[^\circ] \quad und \quad \omega=\frac{^\circ}{s}
\end{equation}
...
\vec{g}*\frac{180}{\pi} \quad mit \quad w=[^\circ]
\end{equation}
...
\begin{equation}
w_{komp}=0.96*w_{Gyro}+0.04*w_{Accl} \quad mit \quad w=[^\circ]
\end{equation}
