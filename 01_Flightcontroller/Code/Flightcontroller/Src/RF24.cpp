...
{
	if(mode == HIGH){
		_cspinx->BSRR = (1<<_cspins);
	}else{
		_cspinx->BSRR = (1<<_cspinr);
	}
}
...
{
	if(level == HIGH){
		_cepinx->BSRR = (1<<_cepins);
	}else{
		_cepinx->BSRR = (1<<_cepinr);
	}
}
...
{
  uint8_t status;
  uint8_t reg_ = R_REGISTER | ( REGISTER_MASK & reg );
  uint8_t dummy = 0xff;

  beginTransaction();
  HAL_SPI_Transmit_DMA(&spi,&reg_,1);
  HAL_SPI_Receive_DMA(&spi,&status,1);
  
  
  while ( len-- ){
	  HAL_SPI_Transmit_DMA(&spi,&dummy,1);
	  HAL_SPI_Receive_DMA(&spi,buf++,1);
  }
...
{
  uint8_t status;
  uint8_t reg_ = W_REGISTER | ( REGISTER_MASK & reg );
  ...
  int i = 0;
  while ( len-- ){
	  HAL_SPI_Transmit_DMA(&spi,(uint8_t *)&buf[i],1);
	  i++;
  }
  while (HAL_SPI_GetState(&spi) != HAL_SPI_STATE_READY);
  endTransaction();
...
uint8_t RF24::write_register(uint8_t reg, uint8_t value)
{
  ...
  uint8_t reg_ = W_REGISTER | ( REGISTER_MASK & reg );
	...
	HAL_UART_Transmit(&huart1,(uint8_t *)buffer,strlen(buffer),(0));
#endif
  beginTransaction();
  HAL_SPI_Transmit_DMA(&spi,&reg_,1);
  HAL_SPI_Receive_DMA(&spi,&status,1);
...
uint8_t RF24::write_payload(const void* buf, uint8_t data_len,  uint8_t writeType)
{
  ...
  int i = 0;
  while ( data_len-- ) {
	  HAL_SPI_Transmit_DMA(&spi,(uint8_t *)current[i],1);
	  i++;
  }
  while ( blank_len-- ) {
	  HAL_SPI_Transmit_DMA(&spi,&blank,1);
  }  
  while (HAL_SPI_GetState(&spi) != HAL_SPI_STATE_READY);
  endTransaction();
...
uint8_t RF24::read_payload(void* buf, uint8_t data_len)
{
  ...
  beginTransaction();
  HAL_SPI_Transmit_DMA(&spi,&reg,1);
  HAL_SPI_Receive_DMA(&spi,&status,1);
  while ( data_len-- ) {
	  HAL_SPI_Transmit_DMA(&spi,&dummy,1);
	  HAL_SPI_Receive_DMA(&spi,current++,1);
  }
  while ( blank_len-- ) {
	  HAL_SPI_Transmit_DMA(&spi,&dummy,1);
  }
...
RF24::RF24(SPI_HandleTypeDef&hspi , GPIO_TypeDef *_cepinx, uint8_t _cepinr, GPIO_TypeDef *_cspinx, uint8_t _cspinr ):
	spi(hspi), _cepinx(_cepinx),_cepinr(_cepinr),_cspinx(_cspinx),_cspinr(_cspinr),
...
{
	uint8_t buf[2] = {ACTIVATE,0x73};
    beginTransaction();
    HAL_SPI_Transmit_DMA(&spi, &buf[0], 1);
    HAL_SPI_Transmit_DMA(&spi, &buf[1], 1);
    while (HAL_SPI_GetState(&spi) != HAL_SPI_STATE_READY);
	endTransaction();
}
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  uint8_t reg = W_ACK_PAYLOAD | ( pipe & 0x07 );
