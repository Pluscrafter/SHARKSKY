...
namespace COM {
	void UART::Init(){
		...
		GPIO::MODE(tx, tx_pin, MODE_ALT);
		GPIO::MODE(rx, rx_pin, MODE_ALT);

		GPIO::AFR(tx, tx_pin, tx_afr);
		GPIO::AFR(rx, rx_pin, rx_afr);


		//uart->CR1 = uart->CR1 | USART_CR1_OVER8;
		uart->BRR = 108000000/115200;//0xEA1;
