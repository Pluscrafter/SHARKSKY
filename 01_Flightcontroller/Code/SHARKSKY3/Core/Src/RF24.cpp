...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
	...
	beginTransaction();
	spi2.TransmitReceive( &temp, &status, 1);
	spi2.Receive( buf, len);
	endTransaction();
...
uint8_t RF24::read_register(uint8_t reg)
{
	...
	uint8_t temp = (R_REGISTER | ( REGISTER_MASK & reg));
	beginTransaction();
	spi2.Transmit( &temp, 1);
	spi2.Receive( &result, 1);
	endTransaction();
...
uint8_t RF24::write_register(uint8_t reg, uint8_t value)
{
	...
	uart1.Transmit((uint8_t *)buffer,strlen(buffer));
#endif
	beginTransaction();
	spi2.TransmitReceive( &temp, &status, 1);
	spi2.Transmit( &value, 1);
	endTransaction();
...
#endif
	beginTransaction();
	spi2.TransmitReceive( &writeType, &status, 1);
	spi2.Transmit( (uint8_t *) current, data_len);
	temp = LOW;
	while (blank_len--)
	{
		spi2.Transmit( &temp, 1);
...
uint8_t RF24::spiTrans(uint8_t cmd)
{
	...
	beginTransaction();
	spi2.TransmitReceive( &cmd, &status, 1);
	endTransaction();
...
void RF24::print_status(uint8_t status)
{
	...
	uart1.Transmit((char *)buffer,strlen(buffer));
}
...
void RF24::print_address_register(const char* name, uint8_t reg, uint8_t qty)
{
	...
	uart1.Transmit((char *)buffer2,strlen(buffer2));
	while (qty--)
	{
...
#endif
/****************************************************************************/
RF24::RF24(GPIO_TypeDef * GPIOce, uint16_t s_bit_ce) :
		GPIOce(GPIOce), s_bit_ce(s_bit_ce), p_variant(false), payload_size(32), dynamic_payloads_enabled(false), addr_width(5), csDelay(5)  //,pipe0_reading_addressHAL_MAX_DELAY
{
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
	...
	beginTransaction();
	spi2.Transmit( &temp, 1);
	spi2.Receive( (uint8_t *) current, data_len);
	endTransaction();
